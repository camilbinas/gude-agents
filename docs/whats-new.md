...
Every invocation can now carry a `Principal` (ID, roles, and arbitrary attributes) that tool-level and agent-level policies inspect before allowing execution. Attach it per-call with `WithPrincipal`, restrict individual tools with `tool.AllowRoles`/`tool.DenyRoles`, and plug in an external authorizer (OPA, Casbin, etc.) via `WithPolicy`. See [RBAC & Identity](rbac.md).
