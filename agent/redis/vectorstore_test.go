package redis

import (
	"context"
	"math"
	"testing"

	"github.com/camilbinas/gude-agents/agent"
	"github.com/camilbinas/gude-agents/agent/rag"
	"pgregory.net/rapid"
)

// genDocument generates a random agent.Document with non-empty Content and 0–3 metadata entries.
func genDocument(t *rapid.T) agent.Document {
	content := rapid.StringMatching(`[a-zA-Z0-9 ]{1,100}`).Draw(t, "content")

	numMeta := rapid.IntRange(0, 3).Draw(t, "numMeta")
	meta := make(map[string]string, numMeta)
	for i := 0; i < numMeta; i++ {
		key := rapid.StringMatching(`[a-z]{1,10}`).Draw(t, "metaKey")
		val := rapid.StringMatching(`[a-zA-Z0-9]{0,20}`).Draw(t, "metaVal")
		meta[key] = val
	}

	return agent.Document{
		Content:  content,
		Metadata: meta,
	}
}

// genEmbedding generates a random float64 embedding of the given dimension, normalized to unit length.
func genEmbedding(t *rapid.T, dim int) []float64 {
	emb := make([]float64, dim)
	var norm float64
	for i := 0; i < dim; i++ {
		v := rapid.Float64Range(-1.0, 1.0).Draw(t, "embVal")
		emb[i] = v
		norm += v * v
	}
	// Normalize to unit vector for cosine similarity.
	mag := math.Sqrt(norm)
	if mag > 0 {
		for i := range emb {
			emb[i] /= mag
		}
	}
	return emb
}

// Feature: redis-providers, Property 2: VectorStore Add/Search Round-Trip
// **Validates: Requirements 7.1, 8.1, 8.4, 9.1, 9.2, 9.3**
func TestProperty_VectorStoreAddSearchRoundTrip(t *testing.T) {
	addr := skipIfNoRedis(t)

	const dim = 128
	indexName := "testidx-roundtrip-" + rapid.StringMatching(`[a-z0-9]{8}`).Example()

	store, err := NewRedisVectorStore(RedisOptions{Addr: addr}, indexName, dim)
	if err != nil {
		t.Fatalf("failed to create RedisVectorStore: %v", err)
	}
	defer store.Close()
	defer store.client.Do(context.Background(), "FT.DROPINDEX", indexName, "DD").Err()

	rapid.Check(t, func(t *rapid.T) {
		doc := genDocument(t)
		emb := genEmbedding(t, dim)

		ctx := context.Background()

		// Add the document.
		if err := store.Add(ctx, []agent.Document{doc}, [][]float64{emb}); err != nil {
			t.Fatalf("Add failed: %v", err)
		}

		// Search with the same embedding.
		results, err := store.Search(ctx, emb, 1)
		if err != nil {
			t.Fatalf("Search failed: %v", err)
		}

		if len(results) == 0 {
			t.Fatal("expected at least one result, got none")
		}

		// The top result should match our document.
		got := results[0].Document

		if got.Content != doc.Content {
			t.Fatalf("content mismatch:\n  expected: %q\n  got:      %q", doc.Content, got.Content)
		}

		// Check that all original metadata key-value pairs are present.
		for k, v := range doc.Metadata {
			gotV, ok := got.Metadata[k]
			if !ok {
				t.Fatalf("metadata key %q missing from result", k)
			}
			if gotV != v {
				t.Fatalf("metadata[%q] mismatch: expected %q, got %q", k, v, gotV)
			}
		}
	})
}

// Feature: redis-providers, Property 3: VectorStore Add Rejects Mismatched Lengths
// **Validates: Requirements 7.2**
func TestProperty_VectorStoreAddRejectsMismatchedLengths(t *testing.T) {
	rapid.Check(t, func(t *rapid.T) {
		n := rapid.IntRange(1, 10).Draw(t, "n")
		m := rapid.IntRange(1, 10).Draw(t, "m")

		// Ensure N != M.
		if n == m {
			if m < 10 {
				m++
			} else {
				m--
			}
		}

		docs := make([]agent.Document, n)
		for i := range docs {
			docs[i] = genDocument(t)
		}

		embeddings := make([][]float64, m)
		for i := range embeddings {
			embeddings[i] = genEmbedding(t, 8)
		}

		store := &RedisVectorStore{}
		err := store.Add(context.Background(), docs, embeddings)
		if err == nil {
			t.Fatalf("expected error for mismatched lengths (docs=%d, embeddings=%d), got nil", n, m)
		}
	})
}

// --- Unit Tests for RedisVectorStore (Task 4.7) ---

// TestNewRedisVectorStore_UnreachableAddr verifies that NewRedisVectorStore returns
// an error containing "ping" when the Redis address is unreachable.
// Validates: Requirement 6.3
func TestNewRedisVectorStore_UnreachableAddr(t *testing.T) {
	_, err := NewRedisVectorStore(RedisOptions{Addr: "localhost:1"}, "testidx", 128)
	if err == nil {
		t.Fatal("expected error for unreachable address, got nil")
	}
	if !contains(err.Error(), "ping") {
		t.Fatalf("expected error to contain 'ping', got: %v", err)
	}
}

// TestRedisVectorStore_SearchTopKZero verifies that Search with topK=0 returns
// a non-nil error.
// Validates: Requirement 8.2
func TestRedisVectorStore_SearchTopKZero(t *testing.T) {
	store := &RedisVectorStore{}
	_, err := store.Search(context.Background(), []float64{1.0, 2.0}, 0)
	if err == nil {
		t.Fatal("expected error for topK=0, got nil")
	}
}

// TestRedisVectorStore_AddEmptySlice verifies that Add with empty docs/embeddings
// slices returns nil without error.
// Validates: Requirement 7.5
func TestRedisVectorStore_AddEmptySlice(t *testing.T) {
	store := &RedisVectorStore{}
	err := store.Add(context.Background(), []agent.Document{}, [][]float64{})
	if err != nil {
		t.Fatalf("expected nil error for empty slices, got: %v", err)
	}
}

// TestRedisVectorStore_DefaultHNSWParams verifies that a newly created RedisVectorStore
// has the default HNSW parameters: M=16 and EF_CONSTRUCTION=200.
// Validates: Requirements 6.7, 6.8
func TestRedisVectorStore_DefaultHNSWParams(t *testing.T) {
	addr := skipIfNoRedis(t)

	indexName := "testidx-defaults-hnsw"
	store, err := NewRedisVectorStore(RedisOptions{Addr: addr}, indexName, 64)
	if err != nil {
		t.Fatalf("failed to create RedisVectorStore: %v", err)
	}
	defer store.Close()
	defer store.client.Do(context.Background(), "FT.DROPINDEX", indexName, "DD").Err()

	if store.hnswM != 16 {
		t.Fatalf("expected default hnswM=16, got %d", store.hnswM)
	}
	if store.hnswEF != 200 {
		t.Fatalf("expected default hnswEF=200, got %d", store.hnswEF)
	}
}

// TestRedisVectorStore_FTCreateIdempotent verifies that calling NewRedisVectorStore
// twice with the same index name succeeds (FT.CREATE is idempotent).
// Validates: Requirement 6.6
func TestRedisVectorStore_FTCreateIdempotent(t *testing.T) {
	addr := skipIfNoRedis(t)

	indexName := "testidx-idempotent"

	store1, err := NewRedisVectorStore(RedisOptions{Addr: addr}, indexName, 64)
	if err != nil {
		t.Fatalf("first NewRedisVectorStore failed: %v", err)
	}
	defer store1.Close()
	defer store1.client.Do(context.Background(), "FT.DROPINDEX", indexName, "DD").Err()

	store2, err := NewRedisVectorStore(RedisOptions{Addr: addr}, indexName, 64)
	if err != nil {
		t.Fatalf("second NewRedisVectorStore failed (should be idempotent): %v", err)
	}
	defer store2.Close()
}

// TestRedisVectorStore_NewRetriever verifies that RedisVectorStore is accepted by rag.NewRetriever.
// Validates: Requirement 6.4
func TestRedisVectorStore_NewRetriever(t *testing.T) {
	addr := skipIfNoRedis(t)
	indexName := "testidx-retriever"
	store, err := NewRedisVectorStore(RedisOptions{Addr: addr}, indexName, 64)
	if err != nil {
		t.Fatalf("failed to create RedisVectorStore: %v", err)
	}
	defer store.Close()
	defer store.client.Do(context.Background(), "FT.DROPINDEX", indexName, "DD").Err()

	// Use a dummy embedder for the test.
	retriever := rag.NewRetriever(dummyEmbedder{}, store)
	if retriever == nil {
		t.Fatal("expected non-nil retriever from rag.NewRetriever")
	}
}

// dummyEmbedder is a minimal Embedder for testing.
type dummyEmbedder struct{}

func (dummyEmbedder) Embed(_ context.Context, _ string) ([]float64, error) {
	return make([]float64, 64), nil
}
